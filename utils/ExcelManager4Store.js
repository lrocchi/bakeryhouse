...
var ExcelManager4Store = {};

ExcelManager4Store.create = function (fromDate, toDate, fileName, store) {
    ...
    // var storeDoc = [];
    // var objIncassi = {};
    // var storePromise = Store.where("active").equals(true).where("_id").equals("store_id").exec(function (err, stores) {
    // storeDoc = stores;
    // });
    ...
    var headerFood = CostType.where("nome").equals("Food").exec(function (err, costTypeDoc) {
        ...
        var FoodCosts = ExcelManager4Store.getCosts(store._id, "Food", fromDate, toDate);
            ...
            Balance.where('store').equals(store._id).where('value').equals(100).where('ref_date').gte(fromDate).where('ref_date').lt(toDate).sort({ ref_date: 1 }).exec(function (err, incassiResults) {
                ...
                if (err) {
                    ...
                    CostType.where("nome").equals("Delivery").exec(function (err, costTypeDoc) {
                        ...
                        var DeliveryCosts = ExcelManager4Store.getCosts(store._id, "Delivery", fromDate, toDate);
                        totaleRiga = 0.0;
...
ExcelManager4Store.getCosts = function (storeID, type, from, to) {
...
module.exports = ExcelManager4Store;